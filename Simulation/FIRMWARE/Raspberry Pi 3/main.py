...
time.sleep(0.5)
lcd_byte(0x01,LCD_CMD) # 000001 Clear display
lcd_string("Smart Helmet ",LCD_LINE_1)
time.sleep(0.5)
while 1:       
	# Print out results
	alcohol_data =  GPIO.input(alcohol_Sensor)
	touch_data =  GPIO.input(touch_Sensor)
	vibration_data =  GPIO.input(vibration_sensor)
	if(vibration_data == False):
		if(touch_data == True):
			lcd_byte(0x01,LCD_CMD) # 000001 Clear display
			lcd_string("Helmet ",LCD_LINE_1)
			lcd_string(" Weared  ",LCD_LINE_2)
			time.sleep(0.5)
			...
			lcd_byte(0x01,LCD_CMD) # 000001 Clear display
			lcd_string("Helmet",LCD_LINE_1)
			lcd_string("not Weared  ",LCD_LINE_2)
			time.sleep(0.5)
	else:
		pio.uart.println("AT")
